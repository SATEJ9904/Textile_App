...
const LoomMsgs = ({ navigation }) => {
    ...
    const HandleFetch = async () => {
        ...
        try {
            ...
            const uniqueResults = Array.from(new Set(result.map(item => item.LoomId)))
                .map(id => result.find(item => item.LoomId === id));
    
            setMessages(uniqueResults);
            console.log(uniqueResults);
    
            const userPromises = uniqueResults.map(item => HandleFetchUser(item.LoomId));
            const usersData = await Promise.all(userPromises);
    
            const filteredUsers = usersData.flat().filter(user => user.LoomOrTrader === 'L');
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.card}>
                            <TouchableOpacity onPress={() => navigation.navigate('YarnTmsg', { yarnId: item.Id })} style={{ padding: '5%' }}>
