...
const KLBookings = ({ navigation, route }) => {
    ...
    const BookedLoomNo = route.params?.BookedLoomNo || bookedLoomNo;
    const fetchData = async () => {
        try {
            ...
            const checkLoomAvailability = async (loom) => {
                const loomId = loom.Id;
                const loomAvailableTo = loom.LoomAvailableTo.date.substring(0, 10);
                console.log(today, loomAvailableTo, loomId)

                const formdata = new FormData();
                formdata.append("Fromdate", today);
                formdata.append("Todate", loomAvailableTo);
                formdata.append("LoomDetailId", loomId);
                ...
                const checkResponse = await fetch("https://textileapp.microtechsolutions.co.in/php/checkbookdate.php", requestOptions);
                const result = await checkResponse.text();
                return { ...loom, availability: result };
            };

            const loomsWithAvailability = await Promise.all(data.map(checkLoomAvailability));

            const availableLooms = loomsWithAvailability.filter(loom => loom.availability === 'Available');

            setLoomNumbers(availableLooms);
            console.log(availableLooms)
            setLoading(false);
        } catch (error) {
