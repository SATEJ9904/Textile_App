...
const ConfirmOrderKnotting = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`https://textileapp.microtechsolutions.co.in/php/getknottingoffer.php?LoomId=${userId}`);
      const json = await response.json();
      const filteredOrders = json.filter(item => item.Orderfinish === null && item.ConfirmLoom===1);
      const sortedOrders = filteredOrders.sort((a, b) => b.KnottingId - a.KnottingId);
  ...
  const startOrder = async (order) => {
    ...
    try {
      const response = await fetch(`https://textileapp.microtechsolutions.co.in/php/updateloomorder.php?LoomOrderId=${order.LoomOrderId}&Confirmed=false`);
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "Yes",
          onPress: async () => {
            console.log(order.KnottingId);
            const formdata = new FormData();
            formdata.append("Id", order.KnottingId);
            formdata.append("ConfirmLoom", "false");
  ...
  const OrderFinished = () => {
    Alert.alert(
      ...
      [
        {
          text: "No",
          onPress: () => console.log("Order completion canceled"),
          style: "cancel",
        },
        {
          text: "Yes",
          onPress: () => {
            console.log(selectedOrder.KnottingId);

            const formdata = new FormData();
            formdata.append("Id", selectedOrder.KnottingId);
            ...
            fetch("https://textileapp.microtechsolutions.co.in/php/finishknotting.php", requestOptions)
              .then((response) => response.text())
              .then((result) => {
                console.log(result);
  ...
  const [modalJobRate, setModalJobrate] = useState(null)
  const ModalDataFetch = async (Id) => {
    console.log(Id)

    try {
      // First API Call
      const response1 = await fetch(
        `https://textileapp.microtechsolutions.co.in/php/getbyid.php?Table=KnottingOffer&Colname=KnottingId&Colvalue=${Id}`
      );
      const data1 = await response1.json();
      console.log(data1[0].TraderId)
      setModalDraft(data1[0].Draft)
      setModalJobrate(data1[0].JobRateRequired)
      if (data1 && data1[0].TraderId) {
        const traderId = data1[0].TraderId;
        ...
        if (data2) {
          setModalData(data2[0]);
          console.log(data2[0]) // Assuming the result is an array and we need the first item
          setLoading(false);
          setModalVisible2(true)
        } else {
          console.error("Error: No data returned from the second API.");
        }
      } else {
        console.error("Error: No TraderId found in the first API response.");
      }
    } catch (error) {
    ...
    <View style={{ backgroundColor: "white", flex: 1 }}>
      ...
      <View style={{ backgroundColor: "#003C43", flexDirection: "row", alignItems: 'center', height: 50, }}>
        ...
        <View style={{ flex: 0.9, alignItems: 'center' }}>
          <Text style={{ fontSize: 26, color: "white", fontWeight: 500 }}> Knotting Orders </Text>
        </View>
                  ...
                  {selectedOrder && (
                    <View>
                      <View>
                        ...
                        <Text style={{ color: "#000", fontSize: 16, fontWeight: "600", marginBottom: "2%", marginTop: "1%" }}>Order No : {selectedOrder.OfferNo}</Text>
                        <Text style={{ color: "#000", fontSize: 16, fontWeight: "600", marginBottom: "2%", marginTop: "0%" }}>Reed : Draft : ReedSpace : {`${selectedOrder.Reed} : ${selectedOrder.Draft} :${+selectedOrder.ReedSpace}`}</Text>
              ...
              <View style={styles.modalContent}>
                ...
                {loading ? (
                  ...
                  <ScrollView contentContainerStyle={styles.scrollViewContent}>
                    {ModalData && (
                      <>
                        <Text style={styles.detailText}><Text style={styles.label}>E-mail:</Text> {ModalData.AppUserId}</Text>
                        <Text style={styles.detailText}><Text style={styles.label}>Name:</Text> {ModalData.Name}</Text>
                        <Text style={styles.detailText}><Text style={styles.label}>Owner Name:</Text> {ModalData.OwnerName}</Text>
                        <Text style={styles.detailText}><Text style={styles.label}>Contact No.:</Text> {ModalData.PrimaryContact}</Text>
                        <Text style={styles.detailText}><Text style={styles.label}>Draft:</Text> {ModalDraft}</Text>
                        <Text style={styles.detailText}><Text style={styles.label}>Job rate:</Text> {modalJobRate}</Text>
                      </>
                    )}
                  </ScrollView>
                )}
